SELECT rel.oid, pr.parname AS name, rel.reltablespace AS spcoid,rel.relacl AS relacl_str,
  ...
  rel.relhassubclass, rel.reltuples::bigint, des.description, con.conname, con.conkey,
    ...
    (SELECT ARRAY(SELECT CASE WHEN (nspname NOT LIKE 'pg\_%') THEN quote_ident(nspname)||'.'||quote_ident(c.relname) ELSE quote_ident(c.relname) END AS inherited_tables
        FROM pg_inherits i
        JOIN pg_class c ON c.oid = i.inhparent
        JOIN pg_namespace n ON n.oid=c.relnamespace
    WHERE i.inhrelid = rel.oid ORDER BY inhseqno)) AS coll_inherits,
  (SELECT count(*) FROM pg_inherits i JOIN pg_class c ON c.oid = i.inhparent JOIN pg_namespace n ON n.oid=c.relnamespace WHERE i.inhrelid = rel.oid) AS inherited_tables_cnt,
...
{% if ptid %}
  regexp_replace(replace(replace(replace(
    pg_get_partition_rule_def(pr.oid, true),
      'START (', 'FOR VALUES FROM ('),
      ') END', ') TO'),
      'VALUES(', 'FOR VALUES IN ('), '(.*) WITH .*', '\1') partition_value,
