...
UNION ALL SELECT 'COMMENT ON ROLE ' || pg_catalog.quote_ident(pg_get_userbyid(%(rid)s::OID)) || ' IS ' ||  pg_catalog.quote_literal(description) || ';' 
     FROM pg_catalog.shobj_description(%(rid)s::OID, 'pg_authid') a(description)
